...
test_that("merge cells test (using rowNumbers/columnNumbers and then from/to)", {
  ...
  # merge the cells and specify new heading
  tbl$mergeCells(rowNumbers=1, columnNumbers=c(2,3))
  cell <- tbl$cells$getCell(1, 2)
  cell$rawValue <- "Item & Qty"
  cell$formattedValue <- "Item & Qty"
  tbl$mergeCells(rFrom=3, cFrom=3, rTo=4, cTo=4)
  cell <- tbl$cells$getCell(3, 3)
  cell$rawValue <- "??"
  cell$formattedValue <- "??"
