...
Cells can be merged.  The range of cells is specified using the row and column of the top-left cell (`rTop` and `cLeft`), then specifying either the number of rows and columns that the merge spans (`rCount` and `cCount`) or the row and column of the bottom-right cell (`rBottom` and `cRight`). 
...
tbl$mergeCells(rTop=1, cLeft=2, rCount=1, cCount=2)
